...
// Define the function to perform image compression
function compressImage(inputImagePath, outputImagePath, quality) {
  // Your existing image compression code (the `compressImageWithSizeReduction` function)
  // ...
  ...
  // Simulate the base64-encoded image (replace with actual encoded image)
...
app.post('/compress-image', upload.single('image'), (req, res) => {
  ...
  const inputImagePath = 'tempImage.jpg';
  ...
  if (isMainThread) {
    ...
    worker.on('message', (message) => {
      if (message.error) {
        console.error('Error compressing image:', message.error);
        res.status(500).send('Image compression failed.');
      } else {
        const base64EncodedImage = message.compressedImage;
        res.json({ compressedImage: base64EncodedImage });
      }
    ...
    worker.postMessage('compressImage');
  } else {
    // This code runs in the worker thread
    const { inputImagePath, outputImagePath, compressionValue } = workerData;

    parentPort.on('message', (message) => {
      if (message === 'compressImage') {
        try {
          const base64EncodedImage = compressImage(inputImagePath, outputImagePath, compressionValue);
          parentPort.postMessage({ compressedImage: base64EncodedImage });
        } catch (error) {
